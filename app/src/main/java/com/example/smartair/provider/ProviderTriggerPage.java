...
public class ProviderTriggerPage extends AppCompatActivity {
    ...
    private List<ProviderDataReading.DateBasedDocument> originalDocuments;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            editTextTrigger = findViewById(R.id.editTextTrigger);
    ...
    private void setupSearchButton() {
        searchButton.setOnClickListener(v -> {
            ...
            String triggerType = editTextTrigger.getText().toString().trim();

            if (!date.isEmpty()) {
                searchByDate(date);
            } else if (!triggerType.isEmpty()) {
                searchByTriggerType(triggerType);
            } else {
                resetToAllData();
            }
        });
    ...
    private void searchByDate(String date) {
        ...
        providerData.searchDateBasedByDate("triggerLogs", date, new ProviderDataReading.DateSearchCallback() {
            @Override
            public void onSuccess(List<ProviderDataReading.DateBasedDocument> documents) {
                updateDisplayWithDocuments(documents, "No trigger logs found for date: " + date);
            }
    ...
    private void searchByTriggerType(String triggerType) {
        if (scrollContent != null) {
            scrollContent.removeAllViews();

            TextView loading = new TextView(this);
            loading.setText("Searching for trigger: " + triggerType + "...");
            scrollContent.addView(loading);
        } else {
            TextView textInfoDisplay = findViewById(R.id.textInfoDisplay);
            textInfoDisplay.setText("Searching for trigger: " + triggerType + "...");
        }

        providerData.searchTriggersByType(triggerType, new ProviderDataReading.DateSearchCallback() {
            @Override
            public void onSuccess(List<ProviderDataReading.DateBasedDocument> documents) {
                if (triggerType.isEmpty()) {
                    updateDisplayWithDocuments(documents, "No trigger logs available");
                } else {
                    updateDisplayWithDocuments(documents, "No trigger logs found for trigger type: " + triggerType);
                }
            }
    ...
    private void updateDisplayWithDocuments(List<ProviderDataReading.DateBasedDocument> documents, String emptyMessage) {
        if (scrollContent != null) {
            ...
            if (documents.isEmpty()) {
                TextView empty = new TextView(ProviderTriggerPage.this);
            ...
            for (ProviderDataReading.DateBasedDocument doc : documents) {
                View widget = widgetFactory.createDateWidget(doc.date, doc.data, "triggerLogs");
            ...
            dataText.append("Search Results:\n\n");
    ...
    private void showError(String message) {
        if (scrollContent != null) {
            scrollContent.removeAllViews();
            TextView error = new TextView(ProviderTriggerPage.this);
    ...
    private void testPermission() {
        providerData.checkChildPermission("triggerLog", new ProviderDataReading.PermissionCallback() {
            @Override
            public void onPermissionResult(boolean hasPermission) {
                if (hasPermission) {
                    loadTriggerData();
                } else {
                    showPermissionText("Permission DENIED\nYou cannot view trigger logs.");
                }
            }
    ...
    private void loadTriggerData() {
        ...
        TextView loading = new TextView(this);
        loading.setText("Loading trigger logs...");
        scrollContent.addView(loading);

        providerData.getDateBasedSubcollection("triggerLogs", new ProviderDataReading.DateBasedSubcollectionCallback() {
            @Override
            public void onSuccess(List<ProviderDataReading.DateBasedDocument> documents) {
    ...
    private void loadTriggerDataSimple() {
        TextView textInfoDisplay = findViewById(R.id.textInfoDisplay);
        ...
        providerData.getDateBasedSubcollection("triggerLogs", new ProviderDataReading.DateBasedSubcollectionCallback() {
            ...
            public void onSuccess(List<ProviderDataReading.DateBasedDocument> documents) {
                originalDocuments = documents;
                if (documents.isEmpty()) {
                    textInfoDisplay.setText("No trigger logs available");
                    return;
                }

                StringBuilder dataText = new StringBuilder();
                dataText.append("Trigger Logs:\n\n");
            ...
            @Override
            public void onFailure(String message) {
                textInfoDisplay.setText("Error loading trigger logs: " + message);
            }
        });
    }
